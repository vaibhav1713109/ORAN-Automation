###############################################################################
##@ FILE NAME:      Error-tag : unknown-attribute
...
class unknown_attribute(vlan_Creation):
    ...
    def session_login(self):
        try:
            ...
            xml_data = """<interfaces xmlns="urn:ietf:params:xml:ns:yang:ietf-interfaces" jhbja="jk">
                ...
                <mac-address xmlns="urn:o-ran:interfaces:1.0">78:7d:f3:01:16:21</mac-address>
                <port-reference xmlns="urn:o-ran:interfaces:1.0">
                    <port-name>ru-port1</port-name>
    ...
    def test_main(self):
        ...
        try:
            ...
            if result != True:
                ...
                STARTUP.ACT_RES(f"{'Error_Tag[unknown-attribute]' : <50}{'=' : ^20}{'PASS' : ^20}",PDF= pdf_log,COL=(105, 224, 113))
                return True    
...
if __name__ == '__main__':
    try:
        ...
        STARTUP.STORE_DATA(
            f"Error occured in line number {exc_tb.tb_lineno}", Format = False,PDF=pdf_log)
        print('Usage: python unknown_attribute.py <Test_Case_ID>')

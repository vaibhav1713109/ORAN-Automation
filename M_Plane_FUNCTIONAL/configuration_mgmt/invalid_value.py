###############################################################################
##@ FILE NAME:      Error-tag : invalid-value
...
class invalid_value(vlan_Creation):
    ...
    def session_login(self):
        try:
            ...
            ###############################################################################
            ## Pre Get filter of interface yang
            ###############################################################################                
            STARTUP.STORE_DATA('********** Interface yang  Get Filter ***********',Format=True,PDF=pdf_log)
            STARTUP.STORE_DATA('>get --filter-xpath /ietf-interfaces:interfaces',Format=True,PDF=pdf_log)
            v_name1 = '''
            ...
            '''
            interface_s = self.session.get(v_name1).data_xml
            ...
            ###############################################################################
            ## Test step for genrating the invalid value
            ###############################################################################                
            xml_data = open("xml/invalid_value.xml").read()
            u1 =f'''
        ...
        ###############################################################################                
            ...
            #STARTUP.STORE_DATA(e.message[0:15])
            if 'Invalid value' in e.message:
    ...
    def test_main(self):
        try:
            ...
            if result != True:
                ...
                STARTUP.ACT_RES(f"{'Error_Tag[invalid-value]' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf_log,COL=(255,0,0))
                return result
                
            else:
                STARTUP.ACT_RES(f"{'Error_Tag[invalid-value]' : <50}{'=' : ^20}{'PASS' : ^20}",PDF= pdf_log,COL=(105, 224, 113))
                return True    
...
if __name__ == '__main__':
    try:
        ...
        STARTUP.STORE_DATA(
            f"Error occured in line number {exc_tb.tb_lineno}", Format = False,PDF=pdf_log)
        print('Usage: python invalid_value.py <Test_Case_ID>')

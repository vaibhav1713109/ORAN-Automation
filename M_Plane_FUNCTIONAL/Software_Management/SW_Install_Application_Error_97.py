...
class M_CTC_ID_SW_D_I_A(vlan_Creation):
    ...
    def test_Main_014(self):
        ...
        ###############################################################################
        self.rmt = configur.get('INFO','sw_path_already_exist_in_ru')
            ...
            if self.session:
                ...
                for key, val in self.RU_Details[1].items():
                    if val[0] == 'true' and val[1] == 'true':
                        ...
                        ## Test Description
                        ###############################################################################
                        Test_Desc = 'Test Description : Test for failed software download due to AUTHENTICATION_ERROR during the process.'
...
def test_m_ctc_id_0SW_App():
    tc014_obj = M_CTC_ID_SW_D_I_A()
    Check = tc014_obj.test_Main_014()
    if Check == False:
        STARTUP.STORE_DATA('{0} FAIL_REASON {0}'.format('*'*20),Format=True,PDF= pdf)
        STARTUP.STORE_DATA('SFP link not detected...',Format=False,PDF= pdf)
        STARTUP.ACT_RES(f"{'O-RU Software Install Application_Error' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf,COL=(0,255,0))
        return False
...
2. Check on Failure O-RU shall send NETCONF install-event notification with the following statuses in case of failures: APPLICATION_ERROR
'''
    ...
    try:
        if Check == True:
            STARTUP.ACT_RES(f"{'O-RU Software Install Application_Error' : <50}{'=' : ^20}{'SUCCESS' : ^20}",PDF= pdf,COL=(0,255,0))
            return True

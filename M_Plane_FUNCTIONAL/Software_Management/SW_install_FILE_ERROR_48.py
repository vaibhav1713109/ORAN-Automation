...
class M_CTC_ID_SW_F_E(vlan_Creation):
    ...
    def test_procedure(self):
        ...
        xml_data = xml_data.format(
            rmt_path=self.rmt_PDF_Format, password=self.du_pswrd, public_key=pub_key)
        ...
        ###############################################################################
        Test_Step4 = '\t\tStep 4 and 5 :  O-RU NETCONF Server sends <notification> with status  FILE ERROR'
    ...
    def test_Main_014(self):
        ...
        ###############################################################################
        self.rmt_PDF_Format = configur.get('INFO','rmt_path_pdf_format')
            ...
            if self.session:
                ...
                for key, val in self.RU_Details[1].items():
                    if val[0] == 'true' and val[1] == 'true':
                        ...
                        ## Test Description
                        ###############################################################################
                        Test_Desc = 'Test Description :  Test to verify whether the downloaded software is not installed due to FILE_ERROR.'
...
def test_m_ctc_id_014():
    tc014_obj = M_CTC_ID_SW_F_E()
    Check = tc014_obj.test_Main_014()
    if Check == False:
        STARTUP.STORE_DATA('{0} FAIL_REASON {0}'.format('*'*20),Format=True,PDF= pdf)
        STARTUP.STORE_DATA('SFP link not detected...',Format=False,PDF= pdf)
        STARTUP.ACT_RES(f"{'O-RU Software Install File_Error' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf,COL=(0,255,0))
        return False
    ...
    Exp_Result = '''Expected Result : 1. Verify the status of the reply message as FAILED. 
2. Check on Failure O-RU shall send software install procedure<install event> fails notification with FILE_ERROR
3. Operation on the file resulted in incompatible file format'''
    STARTUP.STORE_DATA(Exp_Result,Format='DESC',PDF= pdf)

    STARTUP.STORE_DATA('\t\t{}'.format('****************** Actual Result ******************'),Format=True,PDF= pdf)
    try:
        if Check == True:
            STARTUP.ACT_RES(f"{'O-RU Software Install File_Error ' : <50}{'=' : ^20}{'SUCCESS' : ^20}",PDF= pdf,COL=(0,255,0))
            return True

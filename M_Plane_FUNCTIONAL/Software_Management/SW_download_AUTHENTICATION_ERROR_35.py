...
def test_m_ctc_id_014():
    ...
    if Check == False:
        ...
        STARTUP.ACT_RES(f"{'O-RU Software Download AUTHENTICATION_ERROR ' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf,COL=(0,255,0))
        return False
...
2. The O-RU shall send an immediate rpc-reply message with one of following statuses:
        - FAILED   
3. When the O-RU initiates the software download the O-RU shall send NETCONF download-event notification with one of the following statuses: 
    ...
    try:
        if Check == True:
            STARTUP.ACT_RES(f"{'O-RU Software Download AUTHENTICATION_ERROR' : <50}{'=' : ^20}{'SUCCESS' : ^20}",PDF= pdf,COL=(0,255,0))
            return True

...
class M_CTC_ID_SW_F_E(vlan_Creation):
    ...
    def test_procedure(self):
        ...
        Test_Step4 = '\t\tStep 2 :  O-RU NETCONF Server sends <notification> with PROTOCOL_ERROR'
    ...
    def test_Main_014(self):
        ...
        ###############################################################################
        self.rmt = configur.get('INFO','rmt_path_fftp')
            ...
            if self.session:
                ...
                for key, val in self.RU_Details[1].items():
                    if val[0] == 'true' and val[1] == 'true':
                        ...
                        ## Test Description
                        ###############################################################################
                        Test_Desc = 'Test Description :  Software Download Managment when the file system shows PROTOCOL_ERROR.'
...
def test_m_ctc_id_014():
    ...
    if Check == False:
        ...
        STARTUP.ACT_RES(f"{'O-RU Software Install PROTOCOL_ERROR' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf,COL=(0,255,0))
        return False
...
3. When the O-RU initiates the software download failed the O-RU shall send NETCONF download-event notification with one of the following statuses: 
    ...
    try:
        if Check == True:
            STARTUP.ACT_RES(f"{'O-RU Software Install PROTOCOL_ERROR' : <50}{'=' : ^20}{'SUCCESS' : ^20}",PDF= pdf,COL=(0,255,0))
            return True

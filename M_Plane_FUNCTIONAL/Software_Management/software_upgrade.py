###############################################################################
##@ FILE NAME:      Software Upgrade time taken
...
class software_upgrade(vlan_Creation):
...
3. Validate the time taken from the system reboot with the new software image till O-RU comes up.
    '''
                ...
                STARTUP.ACT_RES(f"{'Software Upgrade time taken' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf_log,COL=(255,0,0))
                return result
...
3. Validate the time taken from the system reboot with the new software image till O-RU comes up.  
    '''
                ...
                if check == True:
                    STARTUP.ACT_RES(f"{'Software Upgrade time taken' : <50}{'=' : ^20}{'PASS' : ^20}",PDF= pdf_log,COL=(105, 224, 113))
                    return True  
...
if __name__ == '__main__':
    try:
        ...
        STARTUP.STORE_DATA(
            f"Error occured in line number {exc_tb.tb_lineno}", Format = False,PDF=pdf_log)
        print('Usage: python software_upgrade.py <Test_Case_ID>')

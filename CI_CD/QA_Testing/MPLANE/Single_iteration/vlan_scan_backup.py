...
class vlan_scan(test_DHCP_CONF, test_isc_server_conf,Link_Detect):
    ...
    def read_live_packets_for_incoming_vlan_tag(self,iface = 'wlp0s20f3'):
        ...
        timeout = 3*configur.getint('INFO','booting_complete_time')
        ...
        for pkt in pkts:
            ...
            if val:
                ...
                wrpcap('{}/LOGS/vlan_tag.pcap'.format(parent), pkts)
                return True
        else:
    ...
    def check_ip_in_dhcp_ack_message(self,iface = 'wlp0s20f3'):
        timeout = 2*configur.getint('INFO','booting_complete_time')
        pkts2 = sniff(iface = iface, stop_filter = self.check_dhcp_ack,timeout = timeout)
        for pkt in pkts2:
            val = self.check_dhcp_ack(pkt)
            if val != False:
                STARTUP.append_data_and_print(f'Got DHCP IP {self.hostname} to the VLAN {self.DU_vlan} || Successful',summary)
                wrpcap('{}/LOGS/dhcp.pcap'.format(parent), pkts2)
                time.sleep(2)
                os.system('mergecap -w {0}/LOGS/{1}/vlan_scan.pcap {0}/LOGS/vlan_tag.pcap {0}/LOGS/dhcp.pcap'.format(parent,configur.get('INFO','ru_name_rev')))
                os.system('rm {0}/LOGS/vlan_tag.pcap {0}/LOGS/dhcp.pcap'.format(parent))
                return True
        else:
    ...
    def Call_Home(self):
        ...
        # print("Power OFF || successful.",summary)
        # time.sleep(7)
        # # Turn on O-RU
        # subprocess.run(["curl", "-u", "admin:rpsadmin", f"http://{self.rps_switch_ip}/rps?SetPower=1+1"])
        # print("Power ON || successful.",summary)
        # time.sleep(30)

        self.timeout = configur.getint('INFO','booting_complete_time') - 30

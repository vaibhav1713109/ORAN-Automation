...
def scp_genration(test_case_name,Duplex_Type,eAxID):
    try:
        ...
        api.SaveSettingsFile(f"{root_dir}\\{Duplex_Type}\\Results\\{test_case_name}\\EAXCID{eAxID}\\CTC_5GNR_UL.scp")
...
if __name__ == "__main__":
    print(sys.argv)
    if len(sys.argv)>2:
        print(scp_genration(sys.argv[1],sys.argv[2],sys.argv[3]))
        # print(ORS_instrument_Configuration())
        # input('Enter once sync')
    else:
        print('Please run with below format\npython scp_gen.py {test_case_name} {Duplex_Type} {eaxcid}')
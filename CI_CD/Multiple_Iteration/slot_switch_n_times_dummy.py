...
class Slot_Switch(PDF,netopeer_connection,Configuration,genrate_pdf_report):
    ...
    def Befor_sending_Reset_rpc(self):
        ...
        Check1 = self.check_link_detection()
        ...
        ###############################################################################
        print(f'{"-"*100}\nCheck the status of Static/DHCP ip ping\n{"-"*100}')
        if self.check_static_dhcp_ip_ping(self.hostname):
            self.summary.append([f'Static/DHCP IP {self.hostname} Ping ','Successful'])
            print('-'*100)
            print(f' '.join(self.summary[-1]))
            ping_out = subprocess.getoutput("ping -c 5 {}".format(self.hostname))
        else:
            return f'Static/DHCP IP {self.hostname} not Pinging'
        ...
        try:
            ...
            if self.session:
                ...
                self.STORE_DATA(ping_out,Format=False,)
...
def slot_change(nums):
    ...
    for i in range(nums):
        ...
        # notification(f"{'='*100}\nIteration {i+1}\n{'='*100}")
        print('{0}\nExecution Time is : {1}'.format("="*100,Execution_time))
        smry = ['\n','** Result **'.center(50)]
    ...
    print('\n'.join(smry))
    print(f"{'='*100}\nStatus\t\t=\t{Result}\n{'='*100}")
    print('{0}\n{2} iterations Execution Time is : {1}'.format("="*100,Execution_time,nums))

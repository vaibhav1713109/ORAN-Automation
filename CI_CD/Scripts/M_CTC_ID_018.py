...
class M_CTC_ID_018(vlan_Creation):
    ...
    def test_procedure(self):
        ...
        self.summary['Netconf Session Established!!'] = 'Successfull!!'
        ...
        self.summary['Hello Capabilities Exchanged!!'] = 'Successfull!!'
        ...
        self.summary['Subscription with netconf-config filter Performed!!'] = 'Successfull!!'
        ...
        ## Merge 3 Users
        ###############################################################################
        self.summary['Mearging 3 new user!!'] = 'Successfull!!'
        ...
        while True:
            ...
            try:
                ...
                pass
        self.summary['Configuraion chenged notification captured!!'] = 'Successfull!!'

        ###############################################################################
        ## Test Procedure 3 : Give Configured 3 users diffrent privilege
        ###############################################################################
        self.summary['Configured users privilege for above 3 users!!'] = 'Successfull!!'
        pdf.add_page()
        ...
        if LIST_User != ADDED_USERS_R:
            ...
            STARTUP.STORE_DATA(xml_pretty_str, Format='XML', PDF=pdf)
        self.summary['All 3 user merged and got privileges successfully!!'] = 'Successfull!!'
    ...
    def test_Main_018(self):
        self.summary["Test Case M_CTC_ID_018 is under process..."] = 'Successfull!!'
    ...
    def Main_Function(self):
        ...
        if Check == False:
            ...
            self.summary['FAIL_REASON']= 'SFP link not detected...'
            self.summary['Sudo on Hybrid M-plane Architecture (positive case)'] = 'FAIL'
        ...
        ###############################################################################
        STARTUP.GET_SYSTEM_LOGS(self.hostname,self.USER_N,self.PSWRD,pdf)
        ...
        try:
            if Check == True:
                ...
                self.summary['Sudo on Hybrid M-plane Architecture (positive case)'] = 'Pass'
                return True
                ...
                STARTUP.ACT_RES(f"{'Sudo on Hybrid M-plane Architecture (positive case)' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf,COL=(255,0,0))
                self.summary["FAIL_REASON"] = "{}".format(Error_Info)
                self.summary['Sudo on Hybrid M-plane Architecture (positive case)'] = 'FAIL'
                return False
            else:
                STARTUP.STORE_DATA('{0} FAIL_REASON {0}'.format('*'*20),Format=True,PDF= pdf)
                ...
                STARTUP.ACT_RES(f"{'Sudo on Hybrid M-plane Architecture (positive case)' : <50}{'=' : ^20}{'FAIL' : ^20}",PDF= pdf,COL=(255,0,0))
                self.summary["FAIL_REASON"] = "{}".format(Check)
                self.summary['Sudo on Hybrid M-plane Architecture (positive case)'] = 'FAIL'
                return False
                ...
                STARTUP.STORE_DATA(
                    f"Error occured in line number {exc_tb.tb_lineno}", Format=False,PDF=pdf)
                self.summary["FAIL_REASON : {}".format(e)] = 'Successfull!!'
                self.summary['Sudo on Hybrid M-plane Architecture (positive case)'] = 'FAIL'
                return False
        ...
        ###############################################################################
            ...
            self.summary["Successfully completed Test Case M_CTC_ID_018. Logs captured !!"] = 'Successfull!!'
            self.summary = list(zip(self.summary.keys(),self.summary.values()))
            STARTUP.render_table_data(self.pdf,self.summary)
            STARTUP.CREATE_LOGS('',PDF=self.pdf)


def sudo_on_hybrid():
    start_time = time.time()
    obj1 = M_CTC_ID_018()
    Result = obj1.Main_Function()
    end_time = time.time()
    print('Execution Time is : {}'.format(int(end_time-start_time)))
    print(Result)
    return Result, obj1.summary, int(end_time-start_time)

if __name__ == "__main__":
    Result = sudo_on_hybrid()
    pass

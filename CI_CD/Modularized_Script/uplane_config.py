###############################################################################
##@ FILE NAME:      UPlane Configuration
...
class M_CTC_ID_026(Link_Detect):
    ...
    def uplane_configuration(self):
        try:
            ...
            summary.append('Subscription with netconf-config filter Performed || Successful')
            print('-'*100)
            print(summary[-1])
            ...
            if (ARFCN_RX1 == self.rx_arfcn) and (ARFCN_TX1 == self.tx_arfcn):
                STARTUP.STORE_DATA(xml_pretty_str,Format='XML', PDF=pdf)
                summary.append("o-ran-user-plane yang configured || Successful")
                print('-'*100)
                print(summary[-1])
                return True
            else:
    ...
    def test_Main_026(self):
        ...
        try:
            ...
            ###############################################################################
            print(self.hostname,self.USER_N,self.PSWRD)
            ...
            if self.session:
                ...
                time.sleep(5)
                result = self.uplane_configuration()
                # self.session.close_session()
                if result == True:
                    return True
                else:
...
def Main_Finction():
    ...
    try:
        if Check == True:
            STARTUP.ACT_RES(f"{'UPlane Configuration' : <50}{'=' : ^20}{'SUCCESS' : ^20}",PDF= pdf,COL=[0,255,0])
            summary.append(f"{'UPlane Configuration' : <50}{'=' : ^20}{'PASS' : ^20}")
            return True
            ...
            print('-'*100)
            print(summary[-1])
            summary.append(f"{'UPlane Configuration' : <50}{'=' : ^20}{'FAIL' : ^20}")
            return False
        else:
            STARTUP.STORE_DATA('{0} FAIL_REASON {0}'.format('*'*20),Format=True,PDF= pdf)
    ...
    ###############################################################################
        ...
        STARTUP.render_table_data(pdf,summary)
        STARTUP.CREATE_LOGS('UPlane_Configuration',PDF=pdf)
        summary.append("Successfully completed Test Case UPlane Configuration. Logs captured!!") 
        notification('\n'.join(summary))
...
if __name__ == "__main__":
    ...
    Test_procedure = [f"{'='*100}\nTest case *UPlane Configuration* Started!! Status: Running\n{'='*100}",'** Test Coverage **'.center(50),'Static IP Ping',
                'Netopeer Connection','Capability exchange','Create-subscription','UPlane Configuration', 'Start Time : {}'.format(start_time)]
    notification('\n'.join(Test_procedure))
    Main_Finction()

...
class M_CTC_ID_012(PDF,Configuration,netopeer_connection,genrate_pdf_report):
    ...
    def __init__(self):
        ...
        PDF.PDF_CAP(self)
        self.ptp_port = configur.get('INFO','PTP_SyncE_Port')
        self.P_NEO_IP = configur.get('INFO','P_NEO_IP')
        self.summary = []
    ...
    def test_execution(self,cmd,ethtool_out,ping_output):
        try:
            ...
            if self.session:
                ...
                xml_pretty_sync, dict_data_sync = netopeer_connection.get(self,filter=self.user_filter,cmd=cmd)
                ...
                if sync_state != 'FREERUN':
                    return f'Test_Execution Error : Sync State is {sync_state}'
                ...
                while time.time() < start_time:
                    xml_pretty_sync, dict_data_sync = netopeer_connection.get(self,filter=self.user_filter,cmd='')
    ...
    def Main_Function(self):
        ...
        paragon_ip_ping_status = Configuration.check_ping_status(self.P_NEO_IP)
        # print(type(Check1 and Check2))
        if paragon_ip_ping_status != True:
            return f'Paragon_ip_ping_status ip {self.P_NEO_IP} Failed'

...
class Slot_Switch(Link_Detect):
    ...
    def software_activate(self):
        try:
            self.summary.append(f'Running Software || {self.running_sw}')
            print('-'*100)
            print(self.summary[-1])
            ...
            STARTUP.STORE_DATA(xml_pretty_str, Format='XML', PDF=self.pdf)
            self.summary.append(f'Software {self.running_false} Activate || Successfully activate on {self.inactive_slot}')
            print('-'*100)
            print(self.summary[-1])
            return True
        ...
        while time.time() < t:
            try:
                ...
                if self.session2:
                    ...
                    STARTUP.STORE_DATA(xml_pretty_str, Format='XML', PDF=self.pdf)
                    self.summary.append(f'Running Software after boot || {self.running_false}')
                    print('-'*100)
                    print(self.summary[-1])
        ...
        if Check1 == False:
            ...
            try:
                if Check2 == True:
                    ...
                    if type(Check2) == list:
                        ...
                        STARTUP.ACT_RES(f"{'Slot Switch' : <50}{'||' : ^20}{'FAIL' : ^20}",PDF= self.pdf,COL=(255,0,0))
                        self.summary.append('FAIL_REASON || {}'.format(Check2))
                        print('-'*100)
                        print(self.summary[-1])
                        return False
                ...
                exc_type, exc_obj, exc_tb = sys.exc_info()
                STARTUP.STORE_DATA(f"Error occured in line number {exc_tb.tb_lineno}", Format=False,PDF=self.pdf)
                self.summary.append('Exception || {}'.format(e))
                print('-'*100)
                print(self.summary[-1])
                return False
            ###############################################################################
            ...
            try:
                ...
                if type(Check1) == list:
                    ...
                    STARTUP.ACT_RES(f"{'Slot Switch' : <50}{'||' : ^20}{'FAIL' : ^20}",PDF= self.pdf,COL=(255,0,0))
                    self.summary.append('FAIL_REASON || {}'.format(Check1))
                    print('-'*100)
                    print(self.summary[-1])
                    return False
...
def slot_change():
    ...
    print('-'*100)
    print(f'{st_time}\n{en_time}\n{execution_time}')
    slot_switch.summary.insert(0,'******* Result *******'.center(50))
    slot_switch.summary.insert(0,'='*100)
    notification('\n'.join(slot_switch.summary))
    notification(f'{st_time}\n{en_time}\n{execution_time}')
    notification(f"{'='*100}\nStatus\t\t=\t{status}\n{'='*100}")
    if Result != True:
        sys.exit(1)
    return Result, slot_switch.summary, execution_time
